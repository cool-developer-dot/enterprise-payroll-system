...
export const generatePDF = async (data, options = {}) => {
  ...
  const doc = new PDFDocument({ 
    ...
    info: {
      Title: title,
      Author: metadata.author || 'MeeTech Labs Management system',
  ...
  if (includeFooter) {
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      doc.fontSize(8)
         .font('Helvetica')
         .text(
           `Page ${i + 1} of ${pageCount} | Generated by MeeTech Labs Management system`,
           50,
           doc.page.height - 30,
           { align: 'center' }
         );
...
export const uploadToStorage = async (file, metadata) => {
  ...
  // Construct file URL - files are stored in uploads/{entityType}/filename
  // They are served at /uploads/{entityType}/filename
  // But we also need /api/files/{entityId}/download for download endpoint
  const fileUrl = `/uploads/${entityType}/${path.basename(file.path)}`;
...
export const deleteFromStorage = async (fileId, userId, userRole) => {
  const fileAttachment = await FileAttachment.findById(fileId)
    .populate('uploadedBy', 'role managerId departmentId');
  ...
  if (fileAttachment.status === 'deleted') {
    throw new ResourceNotFoundError('File has already been deleted');
  }

  // Check access permissions
  const isUploader = fileAttachment.uploadedBy._id.toString() === userId.toString();
  const isAdmin = userRole === 'admin';
  const isInAccessUsers = fileAttachment.accessUsers.some(id => id.toString() === userId.toString());
  
  // For managers: allow deletion of employee documents if they manage the employee
  let isManagerWithAccess = false;
  if (userRole === 'manager' && fileAttachment.entityType === 'employee_document') {
    // Check if the manager manages the employee who uploaded the document
    const uploadedByUser = fileAttachment.uploadedBy;
    if (uploadedByUser && uploadedByUser.managerId) {
      isManagerWithAccess = uploadedByUser.managerId.toString() === userId.toString();
    }
    // Also check if the document's entityId (employeeId) matches an employee managed by this manager
    if (!isManagerWithAccess) {
      const employee = await User.findById(fileAttachment.entityId);
      if (employee && employee.managerId && employee.managerId.toString() === userId.toString()) {
        isManagerWithAccess = true;
      }
    }
  }

  // Allow deletion if user is uploader, admin, in accessUsers, or manager with access
  if (!isUploader && !isAdmin && !isInAccessUsers && !isManagerWithAccess) {
...
export const getFilesByEntity = async (entityType, entityId, userId, userRole) => {
  ...
  // Admins and managers can see all files (full access)
  // Employees can see their own files and public files
  if (userRole === 'admin' || userRole === 'manager') {
    // Admin and manager see all files - no additional filtering needed
  } else {
    // Regular users (employees) can only see:
    // 1. Their own uploaded files
    // 2. Public files
    // 3. Files where they have explicit access
    query.$or = [
      { isPublic: true },
  ...
  // Ensure fileUrl is properly formatted (should already be /uploads/entityType/filename)
  // Files are served at /uploads/ so the URL should work as-is
  return files.map(file => ({
    ...file,
    // Ensure fileUrl is accessible (if it's relative, it's already correct)
    // If it needs to be absolute, it will be handled by frontend
    fileUrl: file.fileUrl || `/uploads/${file.entityType}/${file.fileName}`
  }));
};
